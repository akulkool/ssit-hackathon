...
def build_financial_context(df: pd.DataFrame, budget: float) -> str:
    ...
    if df.empty:
        ...
        top_line = ", ".join([f"{r['category']}={money(float(r['amount']))}" for _, r in by_cat.head(3).iterrows()])
    return (
...
def chatbot_answer(prompt: str, df: pd.DataFrame, budget: float) -> str:
    ...
    total = float(df["amount"].sum()) if not df.empty else 0.0
    by_cat = df.groupby("category")["amount"].sum().sort_values(ascending=False) if not df.empty else pd.Series(dtype=float)
    ...
    if "save" in q or "saving" in q:
        ...
        return top_line + "If you cut that by 15-20%, you can redirect the savings into SIP or your emergency fund every month."
    if "most" in q or "highest" in q:
        if by_cat.empty:
            return "I do not see enough data yet. Add a few expenses and ask me again."
        return f"You are spending the most on {by_cat.index[0]} at {money(float(by_cat.iloc[0]))}. We can optimize this first for quick wins."
    if "budget" in q:
        if budget <= 0:
            return "Set a monthly budget and I will actively track usage milestones for you."
        current = float(month_filter(df)["amount"].sum()) if not df.empty else 0.0
        ratio = (current / budget) * 100.0
        return f"Your budget status right now: {ratio:.1f}% used ({money(current)} of {money(budget)}). Want me to suggest a safer weekly cap?"
    if "invest" in q or "sip" in q:
        return "Nice mindset! A simple rule: 50% needs, 30% wants, 20% investing. Start a small SIP now and increase it every quarter."
...
def chatbot(user: dict[str, Any], df: pd.DataFrame) -> None:
    ...
    if prompt:
        ...
        history_msgs = [{"role": r, "content": m} for r, m in st.session_state.chat_history[-10:]]
        st.markdown(
        ...
        answer = ask_openai(history_msgs, financial_context)
        if not answer:
